...
let nameGreeting = nameCheck('Simone');
...
function goldForFrogs(goldToSpend){
  let frogPrice = 3;
  return goldToSpend / frogPrice;
};

let totalFrogs = goldForFrogs(27);
...
function goldForFrogs2(goldToSpend){
  let frogPrice = 3;
  return Math.floor(goldToSpend / frogPrice);
};

let totalFrogs2 = goldForFrogs2(1);
...
function ascendArray(array){

  let flag = false;
  for(let i = 0; i < array.length; i++){
    for(let j = 1; j < array.length; j++){
      if(array[i] < array[j] === true){
        flag = true;
      }else{
        return false;
      }
...
console.log(ascendArray(sampleArray));
